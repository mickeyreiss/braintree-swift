...
public class Braintree {
    ...
    public class var ErrorDomain : String {
        get { return "BraintreeSwiftErrorDomain" }
        set {}
    }
        ...
        if let baseURL = baseURL {
            if let pathComponents = NSURLComponents(URL: baseURL.URLByAppendingPathComponent(path), resolvingAgainstBaseURL: false) {
                ...
                if let URL = pathComponents.URL {
                        ...
                        if let response : NSHTTPURLResponse = response as? NSHTTPURLResponse {
                            ...
                            if let completion = completion {
                                if error != nil {
                                    ...
                                    let error = self.error(Braintree.ErrorCode.APIError, message: "Failed to save credit card")
                    ...
                    let error = self.error(.InternalError, message: "Invalid URL")
                    if let completion = completion {
                        completion(nil, error)
                    }
                    return
